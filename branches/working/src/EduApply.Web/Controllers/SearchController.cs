    ...
    public class SearchController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            else
            {
                ...
                var searchResultQuery = new SearchResultQuery
                {
                    AppNo = searchModel.AppNo,
                    CourseOfStudyId = searchModel.CourseOfStudyId,
                    DateType = searchModel.DateType,
                    DepartmentId = searchModel.DepartmentId,
                    EndDate = searchModel.EndDate,
                    FacultyId = searchModel.FacultyId,
                    FormId = searchModel.FormId,
                    FormTemplateId = searchModel.FormTemplateId,
                    IsAdmitted = searchModel.IsAdmitted,
                    IsPaid = searchModel.IsPaid,
                    IsSubmitted = searchModel.IsSubmitted,
                    Name = searchModel.Name,
                    ProgramId = searchModel.ProgramId,
                    RegNo = searchModel.RegNo,
                    SessionId = searchModel.SessionId,
                    StartDate = searchModel.StartDate,
                    VenueId = searchModel.VenueId
                };
                searchModel = SearchRecord(searchResultQuery);
            }

            // : SearchRecord(Session["searchModel"] as SearchModel);
            return View(searchModel);
        }
        [HttpPost]
        public ActionResult Index(SearchResultQuery query)
        {
            var searchModel = SearchRecord(query);
            return View(searchModel);
        }

        private SearchModel SearchRecord(SearchResultQuery query)
        {
        ...
        public ActionResult PreviewName(string lastName, long? AppId, string AppNum, string firstName, string middleName)
        {
            var appInfo = _searchRepository.GetSearchResult(null, AppNum);
            //string email = appInfo.Select(x => x.Email).Where(x => x. == AppNum).ToString();
            var emailList = appInfo.Where(x => x.AppNum == AppNum).Select(x => x.Email).ToList();
            string email = "";
            if (emailList.Count != 0)
            {
                email = emailList.FirstOrDefault();
                var personalInformation = _registrationService.GetPersonalInformationByEmail(email);
            ...
            {
                ModelState.AddModelError("", "Sorry, the applicant's email does not exixt");
            }
