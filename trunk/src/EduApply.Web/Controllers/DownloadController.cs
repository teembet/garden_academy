...
namespace EduApply.Web.Controllers
{
    ...
    public class DownloadController : Controller
    {
        ...
        public ActionResult CreateExcelForSearch(IEnumerable<string> appNums)
        {
            ...
            row.CreateCell(11).SetCellValue("LGA");
            row.CreateCell(12).SetCellValue("Application Form");
            row.CreateCell(13).SetCellValue("Session");
            row.CreateCell(14).SetCellValue("Faculty");
            row.CreateCell(15).SetCellValue("Department");
            row.CreateCell(16).SetCellValue("Program Course");
            row.CreateCell(17).SetCellValue("O'Level Result");
            row.CreateCell(18).SetCellValue("Educational Details");
            row.CreateCell(19).SetCellValue("Work Experience");
            row.CreateCell(20).SetCellValue("Jamb Score");
            row.CreateCell(21).SetCellValue("Payment Date");
            row.CreateCell(22).SetCellValue("Application Date");
            row.CreateCell(23).SetCellValue("Application Status");
            row.CreateCell(24).SetCellValue("Exam Date");
            row.CreateCell(25).SetCellValue("Exam Venue");
            row.CreateCell(26).SetCellValue("Seat No");
            row.CreateCell(27).SetCellValue("Admission Status");
            row.CreateCell(28).SetCellValue("Submission Date");
            row.CreateCell(29).SetCellValue("Passport");
            int y1 = 1;
            int y2 = 1;
            foreach (var res in result)
            {
                ...
                else
                {
                    ...
                    if (mappedToFormIdz.Any())
                    {
                        foreach (var mappedFormId in mappedToFormIdz)
                        {
                            ...
                            //if (program == null)
                            //{
                            //    progCourse = facultyName = departmentName = "";
                ...
                var applicantsOLevelResult = GetOLevelResultDetails(selectedItem.applicationId);
                var educationalDetails = GetEducationalDetails(selectedItem.applicationId);
                var workEperience = GetWorkExperience(selectedItem.applicationId);
                var country = _locationRepository.GetCountry(selectedItem.country);
                var state = _locationRepository.GetState(selectedItem.state);
                var lga = _locationRepository.GetLocalGovernmentArea(selectedItem.LG);
                rowIndex++;
                row = sheet.CreateRow(rowIndex);
                ...
                row.CreateCell(5).SetCellValue(res.DOB.ToString());
                row.CreateCell(6).SetCellValue(res.gender);
                row.CreateCell(7).SetCellValue(res.phoneNumber);
                row.CreateCell(8).SetCellValue(res.emailAddress);
                row.CreateCell(9).SetCellValue(country != null ? country.Name : "N/A");
                row.CreateCell(10).SetCellValue(state != null ? state.Name : "N/A");
                row.CreateCell(11).SetCellValue(lga != null ? lga.Name : "N/A");
                row.CreateCell(12).SetCellValue(res.applicationForm);
                row.CreateCell(13).SetCellValue(res.Session);
                row.CreateCell(14).SetCellValue(facultyName);
                row.CreateCell(15).SetCellValue(departmentName);
                row.CreateCell(16).SetCellValue(progCourse);
                row.CreateCell(17).SetCellValue(applicantsOLevelResult);
                row.CreateCell(18).SetCellValue(educationalDetails);
                row.CreateCell(19).SetCellValue(workEperience);
                row.CreateCell(20).SetCellValue(res.JambScore);
                row.CreateCell(21).SetCellValue(res.paymentDate);
                row.CreateCell(22).SetCellValue(res.applicationDate);
                row.CreateCell(23).SetCellValue(res.applicationStatus);
                row.CreateCell(24).SetCellValue(res.ExamDate);
                row.CreateCell(25).SetCellValue(res.venue);
                row.CreateCell(26).SetCellValue(res.seatNo);
                row.CreateCell(27).SetCellValue(res.admissionStatus);
                row.CreateCell(28).SetCellValue(res.submissionDate);
                row.CreateCell(29).Row.Height = 1600;
                ...
                var studentPicture = _registrationService.GetPictureDetails(selectedItem.applicationId);
