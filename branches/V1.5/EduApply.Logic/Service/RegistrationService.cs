...
namespace EduApply.Logic.Service
{
    public class RegistrationService : SqlRepository, IRegistrationService
    {
        ...
        public void SaveFeeRequest(FeeRequest feeRequest)
        {
            this.Insert<FeeRequest>(feeRequest);
            this.SaveChanges();
        }


        public FeeRequest GetFeeRequest(string appNum)
        {
            var feeRequest = this.GetAll<FeeRequest>().FirstOrDefault(x => x.ApplicationNumber == appNum);
            return feeRequest;
        }
        public FeeRequest GetFeeRequest(long payeeId, string paymentType)
        {
            var feeRequest = this.GetAll<FeeRequest>().FirstOrDefault(x => x.PayeeID == payeeId && x.PaymentType.ToLower().Equals(paymentType.ToLower()));
            return feeRequest;
        }

        public IEnumerable<FeeRequest> GetFeeRequests()
        {
            var feeRequest = this.GetAll<FeeRequest>();
            return feeRequest;
        }
