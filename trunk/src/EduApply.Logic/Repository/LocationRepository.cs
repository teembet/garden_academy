...
namespace EduApply.Logic.Repository
{
    class LocationRepository : SqlRepository, ILocationRepository
    {
        ...
        public LocationRepository(IDbContext context)
            ...
            {
                var state = this.GetAll<State>().FirstOrDefault(x => x.Name.ToLower().Equals(name.ToLower()));
                return state;
            }


            public LocalGovernmentArea GetLocalGovernmentArea(string name)
            {
                var lga = this.GetAll<LocalGovernmentArea>()
