...
namespace EduApply.Logic.Service
{
    public class PrintService : SqlRepository, IPrintService
    {
        ...
        public string GetPersonalInformation(Data.Entities.PersonalInformation info)
        {
            ...
            htmlText = htmlText.Replace("#NextofKinAddress#", info.AddressOfNextOfkin);
        ...
        public string GetJambBreakDown(string RegNum)
        {
            ...
            if (!string.IsNullOrEmpty(RegNum))
            {
                ...
                if (applicantsJambBreakDown != null || manualApplicantsJambBreakDown != null)
                {
        ...
        {
            string htmlText = "<h3>O'Level Result</h3><hr /><table style='width: 100%'>";
            ...
            if (oLevelDetails.Any())
            {
                ...
                foreach (var detail in oLevelDetails)
                {
                    ...
                    if (counter == 1)
                    {
                        htmlText = htmlText.Replace("#SittingNo#", "FIRST SITTING");
                    }
                    if (counter == 2)
                    {
                        htmlText = htmlText.Replace("#SittingNo#", "SECONG SITTING");
                    }
                    if (counter == 3)
                    {
                        htmlText = htmlText.Replace("#SittingNo#", "THIRD SITTING");
                    }
        ...
        {
            string htmlText = "<h3>Educational Details</h3><hr /><table style='width: 100%' cellspacing='5'>";
            ...
            if (educationalDetails.Any())
            {
                foreach (var detail in educationalDetails)
                {
                    ...
                    htmlText = htmlText.Replace("#GraduationMonth#", detail.GraduationMonth);
        ...
        {
            string htmlText = "<h3>Work Experience</h3><hr /><table style='width: 100%' cellspacing='5'>";
            ...
            if (workExperience.Any())
            {
                foreach (var exp in workExperience)
                {
                    ...
                    htmlText = htmlText.Replace("#ToDate#", exp.ToDate != null ? Convert.ToDateTime(exp.ToDate).ToString("dd-MMM-yyyy") : "Till Date");
        ...
        {
            string htmlText = "<h3>Referees</h3><hr/><table style='width: 100%'  cellspacing='5'>";
        ...
        {
            string htmlText = "<h3>Certificate Upload</h3><hr /><table cellspacing='5'><tr><th style='width: 50%'>Name of Certificate</th><th style='width: 40%'>Type of Certificate:</th></tr>";
        ...
        public string GetProgramCourse(int programId, int courseId)
        {
            ...
            {
                htmlText = "<h3>Program/Course</h3><hr /><table style='width: 100%' cellspacing:'5'>";
                htmlText += "<tr><th style='width:10%'>Program:</th>";
        ...
        public string GetProgramCourse(long applicationId)
        {
            int counter = 1;
            string htmlText = "";
            var applicantsProgramCourse =
                this.GetAll<ApplicantsProgramCourse>().Where(x => x.ApplicationId == applicationId).ToList();
            ...
            if (applicantsProgramCourse.Any())
            {
                ...
                {
                    var program = this.GetAll<Program>().FirstOrDefault(x => x.Id == item.ProgramId);
                    var course = this.GetAll<Course>().FirstOrDefault(x => x.Id == item.CourseId);
                    if (program != null && course != null)
                    {
                        var htmlTemplate = HttpContext.Current.Server.MapPath("~/PrintTemplates/ProgramCoursePreview.html");
                        if (counter == 1)
                        {
                            htmlText += "<tr><th style='width:5%; float: left'>1st</th><th style='width:18.5%; float: left'>Choice Program:</th>" + System.IO.File.ReadAllText(htmlTemplate) + "</tr>";
                        }
                        if (counter == 2)
                        {
                            htmlText += "<tr><th>2nd</th><th>Choice Program:</th>" + System.IO.File.ReadAllText(htmlTemplate) + "</tr>";
                        }
                        if (counter == 3)
                        {
                            htmlText += "<tr><th>3rd</th><th>Choice Program:</th>" + System.IO.File.ReadAllText(htmlTemplate) + "</tr>";
                        }
        ...
        public string GetHeaderAndPassport(long applicationId)
        {
            ...
            string imgPath = HttpContext.Current.Server.MapPath("~/images/" + theme.Logo);
