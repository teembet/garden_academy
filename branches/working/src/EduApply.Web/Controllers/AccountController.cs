...
namespace EduApply.Web.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            {
                ModelState.AddModelError("", "You cannot login now, contact the admin");
            }
            else
            {
        ...
        public ActionResult ForgotPassword(ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                string mail = tenancy.SplashersAdminEamil.ToString();
                _emailSender.SendEmail(model.Email, model.Email, Server.UrlEncode(code), Convert.ToInt32(EmailType.PasswordReset), "");
                _emailSender.SendEmail(mail, model.Email, model.Email, Server.UrlEncode(code), Convert.ToInt32(EmailType.PasswordReset), "");
                //_emailSender.SendEmail(model.Email, model.Email, Server.UrlEncode(code), Convert.ToInt32(EmailType.PasswordReset), "");
