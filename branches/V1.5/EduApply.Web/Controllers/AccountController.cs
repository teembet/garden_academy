...
namespace EduApply.Web.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public AccountController(IEmailSender emailSender, IRegistrationService registrationService, IEncryptionService encryptionService, IAuditTrailRepository auditTrailRepository)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    var auditTrail = new AuditTrail()
                    {
                        ...
                        Details = "Registered on" + DateTime.Now,
                        TimeStamp = DateTime.Now,
                        UserRole = "Student",
                        UserIp = UtilityService.GetIp(System.Web.HttpContext.Current)
                    };
        ...
        public async Task<ActionResult> ConfirmEmail(string user, string code)
        {
            ...
            IdentityResult result = await UserManager.ConfirmEmailAsync(applicationUser.Id, code);
            if (result.Succeeded)
            {
                var userRole = await UserManager.GetRolesAsync(applicationUser.Id);
                var auditTrail = new AuditTrail()
                {
        ...
        [AllowAnonymous]
        [HttpGet]
        public async Task<ActionResult> ConfirmEmailNonApp(string user, string code)
        {
        ...
        [HttpPost]
        public async Task<ActionResult> ConfirmEmailNonApp(CreateApplicantModel model)
        {
            ...
            if (!string.IsNullOrEmpty(model.Password))
            {
                ...
                if (validPass.Succeeded)
                {
                    ...
                    if (result.Succeeded)
                    {
                        IdentityResult confirmResult = await UserManager.ConfirmEmailAsync(applicationUser.Id, model.Code);
        ...
        public ActionResult ResetPassword(string user, string code, string resDt)
        {
            ...
            if (DateTime.Now > resetDate.AddDays(3))
            {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> ApplicantManage(ManageUserViewModel model)
        {
