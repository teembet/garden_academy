...
namespace EduApply.Web.Controllers
{
    ...
    public class UploadController : Controller
    {
        ...
        public ActionResult GetJambListForSession(string sessionId)
        {
            ...
            bool isValid = int.TryParse(sessionId, out outId);

            jambList = _configurationService.GetJambBreakDown(int.Parse(sessionId)).ToList();
            var result = (from s in jambList
                          select new
                          {
                              id = s.Id,
                              RegNum = s.RegNum,
                              ...
                              //fac = s.Faculty,
                              Cos = s.CourseOfStudy,
                              cosCode = s.CourseCode,
                              gender = s.Gender,
                              sor = s.StateOfOrigin,
                              lga = s.LGA,
                              engScore = s.EngScore,
                              subj2 = s.Subject2,
                              subj2score = s.Subject2Score,
                              subj3 = s.Subject3,
                              subj3score = s.Subject3Score,
                              subj4 = s.Subject4,
                              subj4score = s.Subject4Score,
        ...
        public ActionResult GetResultForForm(int? applicationFormId)
        {
            var formResults = new List<FormResult>();


            formResults = _configurationService.GetFormResult(applicationFormId).ToList();
            var result = (from s in formResults
                          select new
                          {
                              id = s.Id,
                              appNum = s.AppNum,
                    ...
                    using (TextReader reader = filestream)
                    {
                        ...
                        while (csv.Read())
                        {
                            try
                            {
                                ...
                                var admittedListEntry = admittedListCollection.FirstOrDefault(x => x.RegNum == regNum);
                                if (admittedListEntry != null)
                                {
                                ...
                                admittedListEntry = admittedListCollection.FirstOrDefault(x => x.AppNum == appNum);
                                if (admittedListEntry != null)
                                {
                                    admittedListCollection.Remove(admittedListEntry);
                    ...
                    if (admittedListCollection.Any())
                    {
                        ...
                        foreach (var item in admittedListCollection)
                        {
                            if (!string.IsNullOrEmpty(item.AppNum))
                            {
                                ...
                                var application = _configurationService.GetApplications(item.AppNum);
                                ...
                                else
                                {
                                    if (string.IsNullOrEmpty(item.RegNum))
                                    {
                                        item.RegNum = application.RegNum;
                                    }
                                ...
                                var applicantsName = personalInformation.LastName + " " + personalInformation.FirstName;
                            ...
                            else
                            {
                                ...
                                application.AdmittedDate = localTime;
                                _registrationService.SaveApplication(application);
                                _emailSender.SendEmail(application.UserName, application.UserName, null, Convert.ToInt32(EmailType.AdmissionOffered), null);
                ...
                if (formResultUpload.ContentLength > 0)
                {
                    ...
                    if (fileExtension != ".csv")
                    {
                        ...
                        var result = new FormResult()
                        {
                            ...
                            ApplicationForms = _applicationFormRepository.GetAppForms().Where(x => x.EndDate < DateTime.Now)
                        };
