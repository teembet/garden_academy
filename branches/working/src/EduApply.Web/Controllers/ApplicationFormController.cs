...
namespace EduApply.Web.Controllers
{
    ...
    public class ApplicationFormController : Controller
    {
        ...
        public ActionResult Create()
        {
            ...
            var educationalDetailsModel = new EducationalDetailsCollection()
            {
                MaxEntry = 0,
                EducationalDetails = new List<EducationalDetails>(),
                ClassOfDegrees = _configurationService.GetDegrees()
            };
            ...
            applicationForm.AgencyCode = tenancy.Code;
            ...
            appFormModel.Sessions = _configurationService.GetSessions().Where(x => x.EndDate > localTime).OrderByDescending(x => x.Name);
            appFormModel.AdmissionLetterFormats = _configurationService.GetAdmissionLetterFormats();
        ...
        public ActionResult Create(ApplicationFormModificationModel model, string startDate, string endDate)
        {
            ...
            if (ModelState.IsValid)
            {
                var applicationForm = new ApplicationForm()
                {
                    ...
                    AdmissionLetterFormatId = model.AdmissionLetterFormatId,
                    Fee = model.Fee,
                ...
                foreach (int templateId in model.IdsToAdd ?? new int[] { })
                {
                    ...
                    switch (formTemplate.Name)
                    {
                        ...
                        case "Certificate_Upload":
                            ...
                            break;
                        case "ProgramCourse":
                            var pcFormTemplateSettings = new FormTemplateSettings()
                            {
                                ApplicationFormId = applicationForm.Id,
                                FormTemplateId = templateId,
                                MinEntry = model.PC_MinEntry,
                                MaxEntry = model.PC_MaxEntry
                            };
                            _appForm.SaveFormTemplateSettings(pcFormTemplateSettings);
                            break;
                    }
        ...
        public ActionResult AdvancedSettings(int formId)
        {
            ...
            model.ClassOfDegrees = _configurationService.GetDegrees();
            model.ApplicationForms = _appForm.GetAppForms().Where(x => x.Id != formId);
            // model.Gateways = _configurationService.GetGateways();
            ...
            // model.GatewayIdzForThisForm = _configurationService.GetAppFormGateways(formId).Select(x => x.GatewayId).ToArray();
            model.ClassOfDegreeIdzForThisForm = _configurationService.GetAppFormClassOfDegrees(formId).Select(x => x.ClassOfDegreeId).ToArray();
            return View(model);
        }
        [HttpPost]
        [Authorize(Roles = "Admin")]
        public ActionResult AdvancedSettings(AdvancedSettingsModel appForm)
        {
            ...
            try
            {
                ...
                applicationForm.DontAllowDiffDeptSelection = appForm.DontAllowDiffDeptSelection;
                applicationForm.UseProgramCourseFromJamb = appForm.UseProgramCourseFromJamb;
                applicationForm.AllowOnlinePayment = appForm.AllowOnlinePayment;
                applicationForm.AllowBankPayment = appForm.AllowBankPayment;
                applicationForm.DontUseDefaultSeatNumbering = appForm.DontUseDefaultSeatNumbering;
                applicationForm.UseDetailsFromJambBiodata = appForm.UseDetailsFromJambBiodata;

                _appForm.Update();


                //configure MappedForm
                ...
                //Configure Gateway
                //var savedGatewayForthisForm = _configurationService.GetAppFormGateways(appForm.Id).ToList();
                //var idsOfSavedGatewayForthisForm = savedGatewayForthisForm.Select(x => x.GatewayId).ToArray();
                //var savedGatewayForThisFormToDelete = savedGatewayForthisForm.Where(x => appForm.GatewayIdz != null && !appForm.GatewayIdz.Contains(x.GatewayId)).ToList();
                //if (appForm.GatewayIdz == null)
                //{
                //    appForm.GatewayIdz = new long[] { };
                //    savedGatewayForThisFormToDelete = savedGatewayForthisForm;
                //}
                //foreach (var item in savedGatewayForThisFormToDelete)
                //{
                //    _configurationService.DeleteAppFormGateway(item);
                //}
                //foreach (var id in appForm.GatewayIdz)
                //{
                //    if (idsOfSavedGatewayForthisForm.Contains(id))
                //    {
                //        continue;
                //    }
                //    var appFormGateway = new AppFormGateway()
                //    {
                //        AppFormId = appForm.Id,
                //        GatewayId = id
                //    };
                //    _configurationService.SaveAppFormGateway(appFormGateway);
                //}

                //Configure Class Of Degree
                var savedCogForthisForm = _configurationService.GetAppFormClassOfDegrees(appForm.Id).ToList();
                var idsOfSavedCogForthisFormForthisFormForthisForm = savedCogForthisForm.Select(x => x.ClassOfDegreeId).ToArray();
                var savedCogForthisFormToDelete = savedCogForthisForm.Where(x => appForm.ClassOfDegreeIdz != null && !appForm.ClassOfDegreeIdz.Contains(x.ClassOfDegreeId)).ToList();
                if (appForm.ClassOfDegreeIdz == null)
                {
                    appForm.ClassOfDegreeIdz = new int[] { };
                    savedCogForthisFormToDelete = savedCogForthisForm;
                }
                foreach (var item in savedCogForthisFormToDelete)
                {
                    _configurationService.DeleteAppFormClassOfDegree(item);
                }
                foreach (var id in appForm.ClassOfDegreeIdz)
                {
                    if (idsOfSavedCogForthisFormForthisFormForthisForm.Contains(id))
                    {
                        continue;
                    }
                    var appFormCog = new AppFormClassOfDegree()
                    {
                        AppFormId = appForm.Id,
                        ClassOfDegreeId = id
                    };
                    _configurationService.SaveAppFormClassOfDegree(appFormCog);
                }
        ...
        public ActionResult Edit(ApplicationFormModificationModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                appForm.AgencyCode = model.AgencyCode;
                ...
                appForm.AdmissionLetterFormatId = model.AdmissionLetterFormatId;
                ...
                var auditTrail = new AuditTrail()
                {
                    ...
                    Username = User.Identity.GetUserName(),
                    AuditActionId = Convert.ToInt32(AuditTrailActions.AddApplicationForm),
                    Details = "edited Application Form \'" + appForm.Name + "\'",
                    TimeStamp = localTime,
                    UserRole = userRole.First(),
                    UserIp = IUtilityService.GetIp()
                };
                ...
                var tempInApp = _appForm.GetFormTemplatesByFormId(model.Id).ToList();
                var tempInAppNotSelected = model.IdsToAdd != null ? _appForm.GetFormTemplatesByFormId(model.Id).Where(x => !model.IdsToAdd.Contains(x.Id)).ToList() : tempInApp;
                foreach (FormTemplate template in tempInAppNotSelected)
                {
                    ...
                    if (formTemplateSettings != null)
                    {
                        _registrationService.DeleteTemplateSettings(formTemplateSettings);
                    }
                    ...
                    if (formTemplateSettings != null)
                    {
                        ...
                        switch (formTemplate.Name)
                        {
                            ...
                            case "Certificate_Upload":
                                ...
                                break;
                            case "ProgramCourse":
                                formTemplateSettings.MinEntry = model.PC_MinEntry;
                                formTemplateSettings.MaxEntry = model.PC_MaxEntry;
                                _appForm.SaveFormTemplateSettings(formTemplateSettings);
                                break;
                        }
                    }
